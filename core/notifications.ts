...
// ---------------------------------------------------------------------------
// Decides WHAT should be reminded about today: overdue / soon-due bills, an
// arriving paycheck, and over-budget categories. Delivery is platform-specific
// (Electron's Notification, Capacitor's LocalNotifications) and lives in the
// respective shells; this module is pure computation over the repo.
// ---------------------------------------------------------------------------
...
export function buildNotes(): Note[] {
  ...
  if (s.notify_paycheck && s.next_paycheck_date) {
    ...
    if (d === 0)
      notes.push({ title: 'Payday!', body: 'Your paycheck is due today — log it when it lands.' });
    else if (d === 1)
      notes.push({ title: 'Paycheck tomorrow', body: 'Your next paycheck arrives tomorrow.' });
  }
